...
func (me *endPoint) needsAideInput() bool {
	...
	for i := 0; i < len(me.exec.inpParams)-1; i++ {
		if me.exec.inpParams[i] == "st:github.com/stupideity/aqua.Aide" {
			panic("Aide parameter should be the last one: " + me.exec.name)
		}
	}
	return me.exec.inpCount > 0 && me.exec.inpParams[me.exec.inpCount-1] == "st:github.com/stupideity/aqua.Aide"
}
...
func (me *endPoint) validateFuncInputsAreOfRightType() {
	if !me.stdHandler {
		for _, s := range me.exec.inpParams {
			switch s {
			case "st:github.com/stupideity/aqua.Aide":
			case "int":
			case "uint":
			case "string":
			default:
...
func (me *endPoint) validateFuncOutputsAreCorrect() {
	...
	if me.httpMethod == "CRUD" {
		...
		if me.exec.outParams[0] != "st:github.com/stupideity/aqua.CRUD" {
...
func (me *endPoint) isAcceptableType(dataType string) bool {
	...
	accepts["string"] = true
	accepts["map"] = true
	accepts["st:github.com/stupideity/aqua.Sac"] = true
	accepts["*st:github.com/stupideity/aqua.Sac"] = true
	accepts["i:."] = true
	accepts["int"] = true
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if e.stdHandler {
			...
			if useCache {
				...
				if err == nil {
					if len(e.exec.outParams) == 2 {
						out = append(out, reflect.ValueOf(http.StatusOK))
						out = append(out, decode(val, e.exec.outParams))
					} else if len(e.exec.outParams) == 1 && e.exec.outParams[0] == "string" {
						out = append(out, decode(val, e.exec.outParams))
					}
					...
					if useCache {
						...
						e.stash.Set(r.RequestURI, out[1].Interface(), ttl)
					}
				}
			} else {
				out = e.exec.Do(ref)
